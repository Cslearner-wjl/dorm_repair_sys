...
- 统一返回：业务成功时 `code=200`，业务失败、参数校验失败和权限错误也通过 JSON `code` 表达，例如 `400`、`401`、`403`、`404`、`409`。当前后端统一异常处理返回 HTTP 200，前端以响应体 `code` 判断业务是否成功。
...
公开注册仅允许创建学生账号。请求体无需传 `role`；如果传入 `ADMIN` 或 `REPAIR`，后端会返回业务错误。
...
```text
sql/init.sql        -- 建库建表
sql/demo-data.sql   -- 插入验收演示数据
```

用途：

- `init.sql` 创建 `dorm_repair` 数据库及全部表结构。
- `demo-data.sql` 清空已有数据，插入三类演示账号、覆盖全部业务状态的工单、处理记录时间线和评价数据。
- 适合答辩前恢复完整的验收数据环境。
